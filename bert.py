...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    out=torch.reshape(out,(bs,seq_len,emb_dim))
    return out
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    """
    step_1=dense_layer(output)
    step_2=dropout(step_1)
    step_3=input+step_2
    out=ln_layer(step_3)
    return out
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # feed forward
    output1=self.interm_dense(output)
    output1=self.interm_af(output1)
    # another add-norm layer
    output=self.add_norm(output,output1,self.out_dense,self.out_dropout,self.out_layer_norm)
    return output
